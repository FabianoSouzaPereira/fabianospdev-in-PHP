...
file_exists('../application/modulos/dao/Login.php')?include_once '../application/modulos/dao/Login.php':include_once 'application/modulos/dao/Login.php';
file_exists('../application/functions/Validador.php')?include_once '../application/functions/Validador.php':include_once 'application/functions/Validador.php';
file_exists('../application/modulos/Crud.php')?include_once '../application/modulos/Crud.php':include_once 'application/modulos/Crud.php';
 ...
 public $insert=null;
    ...
    try{
        ...
        $query="SELECT a.chuId,"
                ...
                ."FROM igrejas as a "
                    ."INNER JOIN relatoriosa as b "
                    ."ON b.relIgreja = a.chuId "
                    ."INNER JOIN celulas as c "
                    ."ON b.relCelula = c.celId "
                ."GROUP BY a.chuId "
                ."HAVING a.chuStatus = '1' "
                ."AND a.chuid IN(SELECT igreja FROM tabela_acesso_igreja WHERE usuario = '$usuId' OR chuLevel >= '$tipo') "
                ."AND c.celStatus = '1' "
    ...
    public function deleteByid($id)
    {
        ...
        try {
            ...
            $query = "UPDATE relatoriosa SET relStatus = :relStatus WHERE relAid= :relAid;";
            $stmt = $conn->getInstance()->beginTransaction();
            $stmt = $conn = new Connection();
            $stmt = $conn->getInstance()->prepare($query);
            $stmt->bindValue(':relAid', $id, PDO::PARAM_INT);
            $stmt->bindValue(':relStatus', '0', PDO::PARAM_STR);
            $stmt->execute();
            ...
            // Failed to update the order into the database so we rollback any changes
    ...
    {
                try{
        $query="SELECT MAX(relAid) FROM relatoriosa;";
        $stmt = $conn = new Connection();
        $stmt = $conn->getInstance()->prepare($query);
        if ($stmt->execute()) {
            while ($raw=$stmt->fetchAll(PDO::FETCH_ASSOC)){
        ...
        try {
            $query="UPDATE relatoriosa "
                    ."SET "
                        ...
                        ."relStatus = :relStatus "
                    ."WHERE relAid =  :relAid "
                    ."AND  relCelula IN(SELECT celulas FROM tabelasacessos WHERE usuario = '$id') "
                    ."AND relIgreja IN (SELECT igreja FROM tabelasacessos WHERE usuario = '$id') "
    ...
    public function pastorReadWholeByid($id_)
    {
        try{
        $tipo=$_SESSION['tipo'];
        $id=$_SESSION['id'];
        $query="SELECT a.`celId`, a.`celuuid`, a.`celCelula`, a.`celRede`, a.`celLider`, a.`celViceLider`, a.`celSecretario`, a.`celAnfitriao`,"
                ...
                ."ON  a.`celId` = b.`relCelula` "
                ."GROUP BY b.relAid "
                ."HAVING  b.relStatus = '1' AND a.celStatus = '1' AND b.`relAid` = '$id_' "
                ."AND b.relCelula IN (SELECT celulas FROM tabelasacessos WHERE usuario = '$id') "
                ."AND b.relIgreja IN (SELECT igreja FROM tabelasacessos WHERE usuario = '$id') "
                ."LIMIT 1;";
        $stmt = $conn = new Connection();
        $stmt = $conn->getInstance()->prepare($query);
                if ($stmt->execute()) {
    ...
    try{
        ...
        $query="SELECT a.`celId`,a.`celCelula`,a.`celStatus`,b.`relAid`, b.`relUid`, b.`relBase`, b.`relMembros`, b.`relVisitantes`,"
                ...
                ."GROUP BY b.relAid "
                ."HAVING  b.relstatus = '1' AND a.celstatus = '1' "
                ."AND celId IN (SELECT celulas FROM celulasdb.tabelasacessos where usuario = '$id') "
                ."LIMIT 1000;";
        ...
        try {
            $query="UPDATE celulasdb.relatoriosa "
                    ."SET "
                        ...
                        ."relStatus = :relStatus "
                    ."WHERE relAid =  :relAid "
                    ."AND  relCelula IN(SELECT celulas FROM celulasdb.tabelasacessos WHERE usuario = '$id') "
                    ."AND relIgreja IN (SELECT igreja FROM celulasdb.tabelasacessos WHERE usuario = '$id') "
                    ."AND relStatus =  :relStatus;";
            $stmt = $conn = new Connection();
            $stmt = $conn->getInstance()->prepare($query);
            $stmt->bindValue(':relAid', $this->getId());
    ...
    public function liderReadWholeByid($id_)
    {
        try{
        $tipo=$_SESSION['tipo'];
        $id=$_SESSION['usuId'];
        $query="SELECT a.`celId`, a.`celuuid`, a.`celCelula`, a.`celRede`, a.`celLider`, a.`celViceLider`, a.`celSecretario`, a.`celAnfitriao`,"
                ...
                ."GROUP BY b.relAid "
                ."HAVING  b.relStatus = '1' AND a.celStatus = '1' AND b.`relAid` = '$id_' "
                ."AND b.relCelula IN (SELECT celulas FROM celulasdb.tabelasacessos WHERE usuario = '$id') "
                ."AND b.relIgreja IN (SELECT igreja FROM celulasdb.tabelasacessos WHERE usuario = '$id') "
                ."LIMIT 1;";
        $stmt = $conn = new Connection();
        $stmt = $conn->getInstance()->prepare($query);
                if ($stmt->execute()) {
