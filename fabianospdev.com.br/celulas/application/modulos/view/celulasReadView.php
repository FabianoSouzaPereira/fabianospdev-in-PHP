...
<table id="tbCelulas" class="listagem table table-bordered table-striped table-responsive"  style="margin-top:10px;">
    ...
    try{
        ...
        if(is_array($ret)){
            Foreach($ret as $raw) {
                $id= $raw['celId']; $celula= $raw['celCelula']; $igreja= $raw['chuIgreja']; $cidade= $raw['chuCidade']; 
                ...
                echo "<td><a href='index.php?page=modulos_view_celulaDetailsView&id=$id' class='btn btn-primary'><span class='glyphicon glyphicon-plus'></span></a></td>";
                echo "<td><a href='index.php?page=modulos_view_celulaUpdateView&id=$id' class='btn btn-primary'><span class='glyphicon glyphicon-edit'></span></a></td>";
                echo "</tr>";
            }
